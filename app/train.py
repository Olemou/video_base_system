...
def parse_arguments():
    ...
    parser.add_argument("--eval_dir", type=str, default="/home/olemou/output_dir",
                        help="Root directory for dataset (must be accessible from all nodes)")
    
    parser.add_argument("--monitoring_dir", type=str, default="/home/olemou/output_dir",
                        help="Output directory for checkpoints (shared across nodes)")
    
    parser.add_argument("--checkpoint_dir", type=str, default="/home/olemou/output_dir",
                        help="Directory for saving checkpoints (shared across nodes)")
    parser.add_argument("--interpreter_dir", type=str, default = "/home/olemou/output_dir",
...
# =============================================================================
def init_distributed_mode(args, logger=None):
    """
    Robust distributed initialization for torchrun (SLURM-safe).
    Works for single-node and multi-node automatically.
    """
    ...
    if "RANK" not in os.environ or "WORLD_SIZE" not in os.environ:
        if logger:
            logger.info(f"Single GPU mode on {socket.gethostname()}")
        else:
            logger.info(f"Single GPU mode on {socket.gethostname()}")
    ...
    args.node_rank = int(os.environ.get("GROUP_RANK", 0))
...
#================================Iterator====================================================
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch + 1) % CHECKPOINT_FREQ == 0 or epoch == (args.num_epochs - 1):
            ...
            if save_every_freq > 0 and (epoch + 1) % 5 == 0:
        ...
        for val_iter in range(steps_val_epoch):
            ...
            def log_stats_eval():
                    ...
                    if (
                        (val_iter % log_freq == 0)
                        or (val_iter == val_iter - 1)
                        or np.isnan(loss_val)
                        or np.isinf(loss_val)
                        ):
                        logger.info(
                            "[%d, %5d] loss: %.3f "
                            % (
            ...
            log_stats_eval()
        logger.info("avg eval loss %.3f" % val_loss_meter.avg)
