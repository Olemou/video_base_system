...
# code from https://github.com/facebookresearch/vjepa2/blob/main/src/datasets/video_dataset.py
...
from src.datasets.augmentation.thermal_augmentation import ThermalAugmentor
import numpy as np
import pandas as pd
import torch
import torchvision
...
def init_data(
    ...
    dataset = VideoDataset(
        ...
        duration=duration,
        fps=fps,
        use_motion_augmentation=use_motion_augmentation,
...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.fps = fps
        self._thermal_aug = ThermalAugmentor()
        ...
        if dataset_fpcs is None:
            self.dataset_fpcs = [frames_per_clip for _ in samples]
        else:
            if len(dataset_fpcs) != len(samples):
                raise ValueError(
